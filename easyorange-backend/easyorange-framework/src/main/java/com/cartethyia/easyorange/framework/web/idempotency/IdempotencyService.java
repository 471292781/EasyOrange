 ...
 * <p>
 * 接收客户端提供的幂等 key，保证同一 key 的业务操作只执行一次：
 * <ul>
 *   <li>已处理 → 返回缓存的成功响应</li>
 ...
 * </p>
 * <p>
 * 执行期间抛出的业务异常不缓存、自动失效，确保重试可重新执行。
 * </p>
 */
public interface IdempotencyService {

    /**
     * 幂等执行一个操作（同一 key 的并发/重复调用只执行一次）。
     *
     ...
     * @throws Exception 业务操作抛出的异常
     */
    <T> T execute(String key, long ttlSeconds, IdempotentOperation<T> operation) throws Exception;
}
