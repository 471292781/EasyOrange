...
class RegisterAppServiceTest {
    ...
    @Mock
    private NicknameGeneratorPort nicknameGenerationPort;
    ...
    @BeforeEach
    void setUp() {
        service = new RegisterAppService(registrationService, nicknameGenerationPort, userEventPort);
    }
    ...
    void register_success() {
        ...
        when(nicknameGenerationPort.generate()).thenReturn(generatedNickname);

        User savedUser = User.builder()
            .id(100L)
            .credentials(new Credentials(username, "encodedPassword"))
            .personalInfo(ImmutablePersonalInfo.builder().nickName(generatedNickname).build())
            .build();
        when(registrationService.registerNewUser(username, password, phone, email, generatedNickname))
            .thenReturn(savedUser);

        Long result = service.register(request);

        assertThat(result).isEqualTo(100L);
        verify(nicknameGenerationPort).generate();
        verify(registrationService).registerNewUser(username, password, phone, email, generatedNickname);
    ...
    void register_withoutContactInfo() {
        ...
        User savedUser = User.builder()
            ...
            .personalInfo(ImmutablePersonalInfo.builder().nickName(generatedNickname).build())
            .build();
        when(registrationService.registerNewUser(username, password, null, null, generatedNickname))
            .thenReturn(savedUser);

        Long result = service.register(request);

        assertThat(result).isEqualTo(101L);
        verify(registrationService).registerNewUser(username, password, null, null, generatedNickname);
