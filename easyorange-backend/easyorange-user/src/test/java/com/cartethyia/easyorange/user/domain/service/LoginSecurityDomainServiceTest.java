...
class LoginSecurityDomainServiceTest {
    ...
    private LoginSecurityDomainService service;

    private static final String ACCOUNT = "testuser";

    @BeforeEach
    void setUp() {
        service = new LoginSecurityDomainService(loginAttemptPort);
    }
    ...
    class RecordFailedAttemptTests {
        ...
        void belowMax() {
            when(loginAttemptPort.incrementAttempts(ACCOUNT)).thenReturn(3L);

            service.recordFailedAttempt(ACCOUNT);

            verify(loginAttemptPort).incrementAttempts(ACCOUNT);
            verify(loginAttemptPort).expireAttempts(ACCOUNT, UserSecurityConstant.ATTEMPTS_EXPIRE_TIME);
        }

        @Test
        @DisplayName("达到上限时抛出锁定异常")
        void reachedMax() {
            when(loginAttemptPort.incrementAttempts(ACCOUNT))
        ...
        void blankAccount() {
            ...
            verify(loginAttemptPort, never()).incrementAttempts(any());
        }
    }
