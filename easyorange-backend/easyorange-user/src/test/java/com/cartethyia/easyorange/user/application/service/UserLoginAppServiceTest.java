...
@DisplayName("UserLoginAppService 测试")
class UserLoginAppServiceTest {
    ...
    private UserLoginAppService service;
    ...
    @BeforeEach
    void setUp() {
        service = new UserLoginAppService(authenticationDomainService, tokenService, userAssembler);
    }
    ...
    void login_password_success() {
        ...
        User user = User.builder()
            ...
            .build();
        when(authenticationDomainService.authenticateByPassword(eq(account), eq(password), anyString()))
            .thenReturn(user);
    ...
    void login_sms_success() {
        ...
        User user = User.builder()
            ...
            .userType(UserType.NORMAL)
            .build();
        when(authenticationDomainService.authenticateBySms(eq(account), eq(verifyCode), anyString()))
            .thenReturn(user);
        when(tokenService.createAccessToken(USER_ID, USERNAME, "01")).thenReturn("access-token");
        when(tokenService.createRefreshToken(USER_ID, USERNAME, "01")).thenReturn("refresh-token");
