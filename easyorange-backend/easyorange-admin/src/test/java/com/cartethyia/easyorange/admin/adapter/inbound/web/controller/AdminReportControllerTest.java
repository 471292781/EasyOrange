...
class AdminReportControllerTest {
    ...
    void listReports_withStatusFilter_shouldFilterByStatus() throws Exception {
        var reports = List.of(new AdminReportResponse(
                "2",
                "101",
                "Product2",
                null,
    ...
    void getReportHistory_shouldReturnHistoryList() throws Exception {
        var history = List.of(ReportHandleHistoryResponse.builder()
                .id("1")
                .reportId("1")
                .operatorName("admin")
                .action("resolve")
                .actionDesc("处理通过")
                .remark("举报已处理")
                .createTime(LocalDateTime.of(2026, 5, 16, 12, 0))
                .build());
    ...
    void batchHandleReports_shouldSucceed() throws Exception {
        ...
        mockMvc.perform(
                        put("/api/admin/reports/batch-handle")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        "{\"reportIds\": [\"1\", \"2\", \"3\"], \"action\": \"dismiss\", \"remark\": \"批量驳回\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("A0000"))
    ...
    void getReportStats_shouldReturnStats() throws Exception {
        var stats = ReportStatsResponse.builder()
                .totalReports(100L)
                .pendingReports(30L)
                .processingReports(10L)
                .resolvedReports(40L)
                .dismissedReports(20L)
                .build();
