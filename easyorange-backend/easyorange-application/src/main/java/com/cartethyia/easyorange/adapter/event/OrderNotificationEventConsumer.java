...
public class OrderNotificationEventConsumer {
    ...
    public void onOrderCreated(OrderCreatedEvent event, Message message) {
        handler.handle(event, message, metadata ->
                sendNotification(event.buyerId(), "订单已创建",
                        "您的订单已创建，订单号: " + event.orderId(), event.orderId()));
    }

    @RabbitHandler
    public void onOrderPaid(OrderPaidEvent event, Message message) {
        handler.handle(event, message, metadata ->
                sendToBuyer(event.orderId(), "订单已支付", "您的订单已支付成功，订单号: " + event.orderId()));
    }

    @RabbitHandler
    public void onOrderShipped(OrderShippedEvent event, Message message) {
        handler.handle(event, message, metadata ->
                sendToBuyer(event.orderId(), "订单已发货", "您的订单已发货，订单号: " + event.orderId()));
    }

    @RabbitHandler
    public void onOrderCompleted(OrderCompletedEvent event, Message message) {
        handler.handle(event, message, metadata ->
                sendToBuyer(event.orderId(), "订单已完成", "您的订单已完成，订单号: " + event.orderId()));
    }

    @RabbitHandler
    public void onOrderCancelled(OrderCancelledEvent event, Message message) {
        handler.handle(event, message, metadata ->
                sendToBuyer(event.orderId(), "订单已取消", "您的订单已取消，订单号: " + event.orderId()));
    }

    @RabbitHandler
    public void onOrderRefunded(OrderRefundedEvent event, Message message) {
        handler.handle(event, message, metadata ->
                sendToBuyer(event.orderId(), "订单已退款", "您的订单已退款，订单号: " + event.orderId()));
    }
