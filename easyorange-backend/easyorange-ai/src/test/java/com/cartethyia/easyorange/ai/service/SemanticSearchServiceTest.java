    ...
    class SearchTests {
        ...
        void search_blankKeyword() {
            SemanticSearchService service = new SemanticSearchService(
                    embeddingModel, Optional.of(searchQueryPort));
        ...
        void search_success() {
            ...
            when(searchQueryPort.search(any(ProductSearchQueryPort.ProductSearchQuery.class)))
                    .thenReturn(new SearchResult(List.of(), 5L, 1, 10,
            ...
            assertThat(result.size()).isEqualTo(10);
            verify(searchQueryPort).search(argThat(q ->
                    q != null && q.queryEmbedding().equals(List.of(0.1f, 0.2f, 0.3f))
                            && q.useSemanticSearch()));
        }
    }
}
