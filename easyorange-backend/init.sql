...
CREATE TABLE `sys_user` (
    ...
    `user_type`       VARCHAR(2)      DEFAULT '01'             COMMENT '用户类型（01=普通用户 02=管理员）',
    `email`           VARCHAR(255)    DEFAULT NULL             COMMENT '邮箱地址，唯一',
    `phonenumber`     VARCHAR(20)     DEFAULT NULL             COMMENT '手机号，唯一',
...
CREATE TABLE `product` (
    ...
    `status`            INT             NOT NULL DEFAULT 0       COMMENT '状态（0=草稿 1=上架 2=已售 3=下架）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='商品详情表（垂直分表）';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='搜索历史归档表';

-- ===================================================================
-- 17.1 领域事件表 (eo_domain_event)
-- ===================================================================
...
ALTER TABLE `product_report` ADD CONSTRAINT `fk_report_product`
    FOREIGN KEY (`product_id`) REFERENCES `product`(`id`) ON DELETE CASCADE;

ALTER TABLE `product_report` ADD CONSTRAINT `fk_report_reporter`
    FOREIGN KEY (`reporter_id`) REFERENCES `sys_user`(`user_id`) ON DELETE CASCADE;

ALTER TABLE `eo_offline_message` ADD CONSTRAINT `fk_offline_user`
    FOREIGN KEY (`user_id`) REFERENCES `sys_user`(`user_id`) ON DELETE CASCADE;

ALTER TABLE `eo_offline_message` ADD CONSTRAINT `fk_offline_message`
    FOREIGN KEY (`message_id`) REFERENCES `eo_message`(`id`) ON DELETE CASCADE;

-- V4 补充的 CHECK 约束
ALTER TABLE `sys_user` ADD CONSTRAINT `chk_user_type`
    CHECK (`user_type` IN ('01', '02'));
