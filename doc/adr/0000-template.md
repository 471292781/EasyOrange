...
7. **位置**：所有 ADR 存放于 `doc/adr/`（相对仓库根）。
8. **索引同步**：新增或状态变更（替代/部分替代）后，必须同步更新 [README.md](README.md) 索引（状态、替代链、日期）。
9. **现状更新**：已「接受」的 ADR 若实现细节漂移（类名/数量/消费者数等），禁止改正文决策内容，在文件顶部加「现状更新（日期）」横幅说明，并同步 README 索引口径（例见 [ADR-0005](0005-messaging-rabbitmq.md)、[ADR-0006](0006-module-decoupling-port-adapter-acl.md)）。
