...
2. **写操作跨模块副作用**：下单要扣库存（`ProductOrderPort`）、订单状态变化要发站内信（`MessageNotifierPort`）、支付要回调校验（`CallbackSignatureVerifierPort`）、AI 估值要查信用（`JdbcCreditScoreFetcher` 走 `CreditScoreFetcher`）。
...
2. **Adapter 归应用模块**：`easyorange-application` 的 `adapter/outbound/` 下集中实现所有跨模块 Port（`ProductOrderAdapter`、`OrderProductQueryAdapter`、`SellerInfoAdapter`、`MessageUserInfoAdapter`、`FavoriteProductInfoAdapter`、admin 四个 `Admin*QueryAdapter` 等），实现类标 `@Primary`（IntelliJ 误报 + 多实现冲突规避，见后端 AGENTS.md 踩坑警示）。
...
- 隔离模式：[ProductOrderPort.java](../../easyorange-backend/easyorange-order/src/main/java/com/cartethyia/easyorange/order/domain/port/ProductOrderPort.java)（order 定义）+ [ProductOrderAdapter.java](../../easyorange-backend/easyorange-application/src/main/java/com/cartethyia/easyorange/adapter/outbound/product/ProductOrderAdapter.java)（application 实现）
