...
8. **异常继承**: 领域异常必须继承 `BaseBusinessException`（common 模块），实现 Spring `ErrorResponse`（`getStatusCode()` 委托 `IResultCode.resolveStatus(code)`，映射单一来源在错误码枚举）。`GlobalExceptionHandler` 使用 Java 21 模式匹配 switch 在单个 `handle()` 方法内按类型分发，业务异常状态码直接取自 `BaseBusinessException#getStatusCode()`（按错误码自动映射：A 段取码内数字推导 4xx，如 A0401/A04011→401/A0403→403/A0404→404/A0405→405/A0429→429，其余 A→400；B→400/C→500/D→502）+ 业务错误码；校验类错误统一返回 400。**禁止直接抛出非 `BaseBusinessException` 子类的 RuntimeException**，否则会落入 500 兜底（IllegalArgumentException 等编程错误一律 500，不做客户端美化）。`BusinessException` 和 `FileException` 构造器均设为 `protected`，抛业务异常时统一使用 `BusinessException.of(...)` / `FileException.of(...)` 工厂方法；子类可正常调用 `super(...)`。各模块领域异常必须使用模块专属 `ResultCode`（如 `ProductResultCode.PRODUCT_NOT_FOUND`），**禁止回退到全局 `B0002`**
...
| 前缀 | 类型 | HTTP 状态 | 示例 |
|------|------|----------|------|
| A | 成功/客户端语义 | 200/4xx | A0000=成功, A0401=未登录, A04011=登录已过期, A0403=禁止访问, A0429=请求过于频繁（限流） |
...
**A 段编号布局**：A + 4 位 HTTP 状态码 + 可选子码数字。A0000 成功；0401=401 未登录 / 04011=401 子码·登录已过期（0402 为 HTTP 402 语义，子码不占用真实 4xx；由 `SecurityConfig#resolveAuthFailureCode` 按 JWT 过期与否区分 A0401/A04011）/ 0403=403 / 0404=404 / 0405=405 / 0429=429。**HTTP 状态由码内数字自动推导**（`IResultCode#resolveStatus` 取 `code[1,5)` 解析，仅接受合法 4xx，否则 400），新增 A04xx 家族码无需改映射。
