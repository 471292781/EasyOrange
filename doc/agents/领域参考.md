...
**已知限制（2026-08-16 记录，暂不修复）**：order 的 `pay` 命令直接置订单 `PAID`，与 payment 模块状态**不联动**——payment 行仍是 PENDING，mock 回调置 SUCCESS 后订单仍是 PENDING_PAYMENT（会被超时取消）。两边状态各自独立，真实网关接入时需补「支付成功 → 订单置 PAID」事件桥接。
