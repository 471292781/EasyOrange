...
- 两个定时任务均按订单加分布式锁（非阻塞，拿不到即跳过）+ 本地事务 + Outbox 事件原子提交；聚合根重建**必须带行项**，否则事件 `productIds` 为空导致库存恢复/售出标记静默失效
- 订单完成事件触发 `ProductInventoryPort.markAsSold`（商品 `ONLINE → SOLD`）；取消/退款事件触发 `restoreStock` 恢复库存
- 下单链路（锁 → 准备 → 建单 + Outbox → 扣库存 → 建支付，单事务原子提交，拒绝 Saga）见 [ADR-0007](../adr/0007-order-local-tx-over-saga.md)

**支付-订单状态联动（2026-08-16 修复）**：订单置 `PAID` 唯一由「支付成功」事件驱动——`PUT /api/orders/{id}/pay` 校验买家与 `canPay()` 后经 `PaymentGatewayPort.pay` 委托支付模块两阶段支付（PENDING → PAYING → SUCCESS），不再直接置 PAID；`Payment.confirmPay` 发布 `PaymentSucceededEvent`（含 orderId，Outbox 同事务原子）→ RabbitMQ `payment.succeeded` → order 模块 `PaymentSucceededEventConsumer` → `OrderCommandHandler.handlePaymentSucceeded` 经 `PAY` 守卫置 `PAID` 并发布 `OrderPaidEvent`。mock 回调（`/mock-payment/success`、`/mock-payment/process`）与真实网关回调（`POST /api/payments/callback`）同路径发布事件。消费按 eventId 幂等（`EventConsumerHandler`）；订单已支付时跳过。
**取消补偿（2026-08-16 实现）**：支付成功事件到达时订单已取消（超时/买家取消/管理端强制取消）→ 订单保持 `CANCELLED` 不再流转（库存已在取消时恢复），`handlePaymentSucceeded` 触发自动退款：`PaymentGatewayPort.refundPayment(orderId, "支付成功但订单已取消，自动退款")` → payment 模块两阶段退款（SUCCESS → REFUNDING → REFUNDED，`PaymentRefundedEvent`）；退款失败仍经容器重试进 DLQ/terminal 人工介入。订单已 `REFUNDED` 等其余非法状态仍由 `PAY` 守卫抛 `ORDER_STATUS_ERROR` 走 DLQ/terminal（防重复支付事件重复退款）。
