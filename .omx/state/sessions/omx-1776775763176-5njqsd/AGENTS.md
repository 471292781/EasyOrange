...
**Stack:** Spring Boot 4 (backend) + Next.js 15 App Router (frontend)
**Architecture:** Multi-module Maven monorepo (backend) + Next.js monorepo (frontend)
**State:** 技术栈升级中
...
```
EasyOrange/
├── easyorange-backend/          # Spring Boot 4 multi-module
...
├── easyorange-frontend/         # Vanilla TS + Vite SPA (legacy, transitioning to Next.js)
│   ├── src/                      # TypeScript source
│   │   ├── api/                 # Fetch-based API client
│   │   ├── components/           # Vanilla TS components
│   │   ├── pages/               # Page modules (home, products, profile, etc.)
│   │   ├── styles/              # BEM CSS + CSS variables
│   │   ├── types/               # Shared TypeScript types
│   │   └── utils/               # Helper functions
│   ├── index.html               # Home page entry
│   ├── products.html            # Products listing
│   ├── profile.html             # User profile page
│   ├── publish.html             # Publish product page
...
| Frontend pages | `frontend/src/pages/` | Vanilla TS page modules |
| Frontend components | `frontend/src/components/` | Vanilla TS component modules |
| Frontend API layer | `frontend/src/api/` | Fetch-based API client |
| Frontend state | Vanilla TS (no framework) | Module-level state in page files |
...
- DTOs: Use Java Records for immutable DTOs (`dto/request/`, `dto/response/`, `dto/vo/`)
- Return type: `Result<T>` from easyorange-common
- Validation: Use `@Valid` + Jakarta validation annotations
- **Business validation: Use `BizRequire` for all business/parameter validation (MANDATORY)**
- Password: BCrypt encoding via `PasswordEncoder`
- Auth: JWT Bearer token, stateless session
- **Constructor injection only** — no field injection
- Use `Pageable` and `PageRequest` for paginated queries
- Use `@Transactional(readOnly = true)` for read operations
- Exception handling: `@ControllerAdvice` for centralized error handling
- Logging: SLF4J with structured logging, use `log.info("action field=value", value)` pattern
- **MyBatis-Plus**: Use MP's built-in null-value filtering, NOT `Optional.ofNullable().ifPresent()`
- **Class structure order**: static constants → fields → constructors → override methods → core business methods → private methods → getters/setters
- **Core business methods order**: query → create → update → delete → other
- **Tool class priority**: Spring native → Apache Commons Lang3 → JDK native, **NO Hutool**

### Frontend (Vanilla TypeScript + Vite)
- **SPA**: Multi-page app with vanilla TypeScript, no framework
- **Component patterns**: Class-based TS components with DOM manipulation
- **State management**: Module-level variables and closures (no state library)
- **Data fetching**: Native Fetch API via centralized API modules in `src/api/`
- **Forms**: Manual validation with helper utilities
- **CSS**: BEM naming convention + CSS custom properties for theming
- **TypeScript**: Strict mode, no `as any`, use proper types instead

## ANTI-PATTERNS (THIS PROJECT)

### Backend
- NEVER log passwords or sensitive data (excluded via `@Log` annotation)
- NEVER return password hashes in API responses
- NEVER use field injection — always use constructor injection
- NEVER use `@Transactional` without specifying read-only flag
- NEVER catch generic `Exception` without proper handling

### Frontend
- **No `as any` or `@ts-ignore`** in TypeScript code
- **No framework imports** — vanilla TS only (no React, Vue, etc.)
- **No direct DOM spaghetti** — use component classes, not inline manipulation
- **No prop drilling** — use event delegation or shared state modules
- **No large monolithic files** — pages should be under 300 lines
- **No `!important` in CSS** — use proper specificity and BEM naming
- **No redundant server state** — API responses are used directly without caching layer
...
- **Java**: 25 (defined in pom.xml)
- **Spring Boot**: 4.x
- **Frontend**: Vanilla TypeScript + Vite SPA (transitioning from legacy Vanilla TS to Next.js 15)
- **Node.js**: 20+

### Development
- Frontend Vite dev server runs on `:5173`, proxies `/api` to backend `:8080`
- Backend runs on `:8080` with CORS configured for frontend origin
...
- Use native Fetch API for client-side data fetching
- **Test coverage target**: 80%+ line coverage
- **Error codes**: Use centralized error code definitions

<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1776775763176-5njqsd | 2026-04-21T12:49:23.314Z

**Codebase Map:**
  easyorange-frontend/: playwright.config, activityApi, request, favoriteApi, insuranceApi, messageApi, orderApi, paymentApi, productApi
