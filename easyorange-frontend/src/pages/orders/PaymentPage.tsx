import { ArrowLeft, CreditCard, Loader2, Shield, Smartphone, Wallet } from 'lucide-react';
...
function PaymentPage() {
    ...
    const { data: paymentStatus } = usePaymentStatus(paymentId ?? '', paymentId !== null);
    ...
    return (
        <div className="payment-page">
            ...
            <div className="payment-amount-section">
                ...
                <p className="payment-amount-value">¥{order?.totalAmount?.toFixed(2) ?? '0.00'}</p>
                {order?.items?.[0] && (
            ...
            <div className="payment-footer">
                <div className="payment-footer-amount">
                    <span>需支付：</span>
                    <span className="payment-footer-price">¥{order?.totalAmount?.toFixed(2) ?? '0.00'}</span>
                </div>
                <Button className="payment-submit-btn" onClick={handleSubmit} disabled={isSubmitting || isPaying}>
                    {isPaying ? (
                        <>
