...
import { passwordLoginSchema, smsLoginSchema, registerSchema } from '@/schemas/authSchema'
import './login.css'
...
function LoginPage() {
  ...
  const { register: registerField, handleSubmit, watch, setValue } = useForm({
    defaultValues: {
      account: '',
      password: '',
      confirmPassword: '',
  ...
  const handleSendSmsCode = async () => {
    const phoneResult = smsLoginSchema.shape.phone.safeParse(vals.account)
    if (!phoneResult.success) {
      addToast({ type: 'error', message: phoneResult.error.issues[0].message })
      return
    }
    setIsSendingCode(true)
    try {
      await userApi.sendSmsCode(vals.account)
      startCountdown()
  ...
  const onFormSubmit = handleSubmit(async () => {
    ...
    if (activeTab === 'login') {
      if (loginMethod === 'password') {
        const pwResult = passwordLoginSchema.safeParse({ account: vals.account, password: vals.password })
        if (!pwResult.success) {
          addToast({ type: 'error', message: pwResult.error.issues[0].message })
          return
        }
        ...
        const smsResult = smsLoginSchema.safeParse({ phone: vals.account, smsCode: vals.smsCode })
        if (!smsResult.success) {
          addToast({ type: 'error', message: smsResult.error.issues[0].message })
          return
        }
      ...
      const regResult = registerSchema.safeParse({
        username: vals.account,
        password: vals.password,
        confirmPassword: vals.confirmPassword,
        agreeTerms: vals.agreeTerms,
      })
      if (!regResult.success) {
        addToast({ type: 'error', message: regResult.error.issues[0].message })
        return
      }
      ...
      try {
        ...
        setRegisteredUsername(vals.account)
  ...
  return (
    <div className="auth-page-container">
      <Button variant="ghost" size="icon" className="auth-page-close-btn" onClick={() => navigate('/')} aria-label="关闭登录页">
      ...
      <div className="auth-page-modal">
        ...
        <div className="auth-page-form-panel">
          ...
          {activeTab === 'login' && (
            <form className="auth-page-form auth-form-entrance" onSubmit={onFormSubmit}>
              ...
              <div className="auth-page-input-group">
                <div className="relative flex items-center">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground pointer-events-none">
                    {loginMethod === 'password' ? (
                      <>
                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
                        <circle cx="12" cy="7" r="4"/>
              ...
              {loginMethod === 'password' ? (
                <div className="auth-page-input-group auth-field-enter" key="login-password-field">
                  ...
                  {capsLock && (
                    <div className="caps-lock-warning">⚠ 大写锁定已开启，请注意密码大小写</div>
                  )}
                </div>
              ) : (
                <div className="auth-page-input-group auth-field-enter" key="login-sms-field">
                  <div className="relative flex items-center">
                    ...
                    <Button
                      ...
                      disabled={countdown > 0 || isSendingCode || !vals.account}
                    >
              ...
              <div className="auth-page-form-options">
                {loginMethod === 'password' ? (
                  <>
                    ...
                    <Button type="button" variant="link" className="auth-page-forgot-link" onClick={() => navigate('/forgot-password')} data-testid="link-forgot-password">忘记密码？</Button>
                  </>
                ) : (
                  <Button type="button" variant="link" className="auth-page-forgot-link" onClick={() => handleLoginMethodChange('password')}>使用密码登录</Button>
                )}
              </div>
              ...
              <div className="auth-page-social-login">
                <Button type="button" variant="outline" size="icon" className="auth-page-social-btn social-btn--wechat" aria-label="微信登录" onClick={() => addToast({ type: 'info', message: '微信登录功能开发中，敬请期待' })}>
                ...
                <Button type="button" variant="outline" size="icon" className="auth-page-social-btn social-btn--qq" aria-label="QQ 登录" onClick={() => addToast({ type: 'info', message: 'QQ登录功能开发中，敬请期待' })}>
                ...
                <Button type="button" variant="outline" size="icon" className="auth-page-social-btn social-btn--weibo" aria-label="微博登录" onClick={() => addToast({ type: 'info', message: '微博登录功能开发中，敬请期待' })}>
              ...
              <div className="auth-page-form-footer">
                <p>还没有账户？<Button type="button" variant="link" className="auth-page-switch-link" onClick={() => setActiveTab('register')}>立即注册</Button></p>
              </div>
            </form>
          )}
          ...
          {activeTab === 'register' && (
            <form className="auth-page-form auth-form-entrance" onSubmit={onFormSubmit}>
              ...
              <div className="auth-page-form-footer">
                <p>已有账户？<Button type="button" variant="link" className="auth-page-switch-link" onClick={() => setActiveTab('login')}>立即登录</Button></p>
              </div>
            </form>
          )}
        </div>
