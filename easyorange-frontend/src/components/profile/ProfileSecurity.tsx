    ...
    const securityItems = [
        {
            ...
            status: '已设置',
            icon: KeyRound,
            action: onShowPasswordModal,
        },
        {
            title: '手机绑定',
            desc: user?.phone ? `已绑定 ${user.phone}` : '绑定手机可提高安全性',
            status: user?.phone ? '已绑定' : '未绑定',
            icon: Phone,
            action: () => !user?.phone && onEdit('phone', ''),
        },
        {
            title: '邮箱绑定',
            desc: user?.email ? `已绑定 ${user.email}` : '绑定邮箱可接收通知',
            status: user?.email ? '已绑定' : '未绑定',
            icon: Mail,
            action: () => !user?.email && onEdit('email', ''),
        },
        {
            title: '实名认证',
            desc: user?.realName ? `已认证 ${user.realName}` : '完成实名认证可提升信任度',
            status: user?.realName ? '已认证' : '未认证',
            icon: Shield,
            action: () => !user?.realName && onEdit('realName', ''),
        },
    ];
                        ...
                        return (
                            ...
                            <div className="setting-item" key={index}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                    <span className={`status-badge ${isActive ? 'active' : ''}`}>{item.status}</span>
