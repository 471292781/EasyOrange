...
describe('MessageBubble', () => {
  ...
  it('shows recalled text when type is RECALLED', () => {
    render(
      <MessageBubble
        message={{ ...baseMessage, type: 'RECALLED', status: 'SENT', content: '' }}
        isOwn={false}
      />,
    );
  ...
  it('shows recalled text when status is RECALLED', () => {
    render(
      <MessageBubble
        message={{ ...baseMessage, status: 'RECALLED' }}
        isOwn={false}
      />,
    );
    expect(screen.getByText('[消息已撤回]')).toBeInTheDocument();
  ...
  it('shows SENDING dots for sending status', () => {
    const { container } = render(
      <MessageBubble message={{ ...baseMessage, status: 'SENDING' }} isOwn={true} />,
    );
  ...
  it('shows FAILED icon for failed status', () => {
    const { container } = render(
      <MessageBubble message={{ ...baseMessage, status: 'FAILED' }} isOwn={true} />,
    );
  ...
  it('shows checkmark for SENT status on own message', () => {
    const { container } = render(
      <MessageBubble message={{ ...baseMessage, status: 'SENT' }} isOwn={true} />,
    );
  ...
  it('shows checkmark for DELIVERED status on own message', () => {
    const { container } = render(
      <MessageBubble message={{ ...baseMessage, status: 'DELIVERED' }} isOwn={true} />,
    );
    const checkmarks = container.querySelectorAll('svg');
    expect(checkmarks.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('does not show context menu when message is recalled', () => {
    render(
      <MessageBubble
        message={{ ...baseMessage, type: 'RECALLED', content: '' }}
        isOwn={false}
      />,
    );
