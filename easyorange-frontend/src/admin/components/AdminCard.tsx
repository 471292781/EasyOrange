...
export function AdminCard({ title, extra, children, className, bodyClassName, noPadding = false }: AdminCardProps) {
    return (
...
export function AdminCardStats({ title, value, icon, trend, className }: AdminCardStatsProps) {
    return (
        <Card
            className={cn(
                'rounded-2xl border border-border/60 bg-white p-5 shadow-md transition-all hover:-translate-y-1 hover:shadow-lg',
                className
            )}
        >
