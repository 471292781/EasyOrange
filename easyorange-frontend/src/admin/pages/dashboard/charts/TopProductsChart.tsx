...
const BAR_COLORS = ['#F97316', '#FB7185', '#C39BD3', '#FBBF24', '#10B981',
                    '#F97316', '#FB7185', '#C39BD3', '#FBBF24', '#10B981'];

export default function TopProductsChart({ data }: TopProductsChartProps) {
  if (data.length === 0) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 260, color: '#B5AEA8', fontSize: '0.87rem' }}>
        暂无商品数据
      </div>
    );
  }
    ...
    <ResponsiveContainer width="100%" height={Math.max(200, data.length * 32)}>
      ...
      >
        ...
        <Tooltip
          ...
          formatter={(value: number, _name: string, props: { payload: { fullName: string; price: number } }) => {
            return [`浏览量: ${value.toLocaleString()} | ¥${props.payload.price?.toFixed(2) ?? '-'}`, props.payload.fullName];
          }}
          labelFormatter={() => ''}
        />
