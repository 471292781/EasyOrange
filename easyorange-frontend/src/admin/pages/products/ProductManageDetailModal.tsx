...
export function ProductManageDetailModal({ open, productId, onClose, onSuccess }: ProductManageDetailModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) {onClose();} }}>
      <DialogContent
        className="[&>button]:hidden flex max-h-[calc(100vh-2rem)] w-[calc(100%-2rem)] max-w-[520px] flex-col gap-0 overflow-hidden rounded-3xl border border-white/70 bg-white/92 p-0 shadow-[0_24px_64px_rgba(42,37,32,0.18),0_8px_24px_rgba(249,115,22,0.06)]"
            ...
            <div className="flex flex-col gap-5">
              ...
              {images.length > 0 && (
                <div className="flex flex-col gap-[0.65rem]">
                  <div
                    ...
                    style={{ aspectRatio: '16/10', cursor: images[selectedImage] ? 'pointer' : 'default' }}
                  ...
                  >
                    {images[selectedImage] ? (
                      <img src={images[selectedImage]} alt={productData.name} className="h-full w-full object-cover" />
                    ) : (
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {images.map((img, index) => (
                        <ShadcnButton
                          ...
                          style={{
                            width: 52, height: 52, minHeight: 'unset',
              ...
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  {productData.originalPrice && productData.originalPrice > (productData.price ?? 0) && (
                    <span className="text-[0.85rem] text-[#B5AEA8] line-through">{formatPrice(productData.originalPrice)}</span>
                  )}
                </div>
              </div>
              ...
              {productData.description && (
                <div className="rounded-[14px] border border-[rgba(229,224,219,0.35)] bg-[linear-gradient(135deg,rgba(249,115,22,0.03),rgba(195,155,211,0.02))] px-4 py-[0.85rem]">
                  ...
                  <p className="whitespace-pre-wrap text-[0.87rem] leading-relaxed text-[#4A4540]">{productData.description}</p>
                </div>
              )}
        ...
        {productData && actions.length > 0 && (
          <div className="flex gap-[0.65rem] border-t border-[rgba(229,224,219,0.4)] bg-[linear-gradient(180deg,rgba(250,248,245,0.5),rgba(250,248,245,0.9))] px-6 py-4">
            {actions.map((action) => (
              ...
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="mr-[0.35rem]">
                  {action.variant === 'online' ? (
