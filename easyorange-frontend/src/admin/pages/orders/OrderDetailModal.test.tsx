...
const sampleOrderDetail: AdminOrderDetail = {
  ...
  product: { productId: 10, name: '测试商品', mainImage: null, price: 100.00 },
  amount: 100.00,
  status: 0,
  statusDesc: '待付款',
  paymentStatus: 0,
...
describe('OrderDetailModal', () => {
  ...
  it('does not render when open is false', () => {
    const { container } = renderWithProviders(
      <OrderDetailModal open={false} orderId={null} onClose={() => {}} />,
    );
  ...
  it('does not render when orderId is null', () => {
    const { container } = renderWithProviders(
      <OrderDetailModal open={true} orderId={null} onClose={() => {}} />,
    );
    expect(container.textContent).toBe('');
  });
  ...
  it('shows loading spinner when loading', () => {
    ...
    renderWithProviders(
      <OrderDetailModal open={true} orderId={1} onClose={() => {}} />,
    );
  ...
  it('shows not found message when order is undefined', () => {
    ...
    renderWithProviders(
      <OrderDetailModal open={true} orderId={999} onClose={() => {}} />,
    );
