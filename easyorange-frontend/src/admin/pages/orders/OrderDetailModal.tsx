      ...
      >
        ...
        <DialogHeader className="relative flex-row items-center justify-between border-b border-[rgba(229,224,219,0.5)] px-6 py-5 text-left">
          <div
            ...
            style={{ background: 'linear-gradient(90deg, rgba(251,191,36,0.12), rgba(195,155,211,0.08), transparent)' }}
          />
          <DialogTitle
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex items-center gap-4 rounded-2xl border border-[rgba(251,191,36,0.08)] bg-[linear-gradient(135deg,rgba(251,191,36,0.05),rgba(249,115,22,0.03))] p-4">
                ...
                <div className="min-w-0 flex-1">
                  <span className="text-[1.35rem] font-bold text-[#EA580C]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              ...
              {orderData.items?.length === 1 ? (
                (() => {
                  ...
                  return (
                    <div className="flex gap-[0.85rem] rounded-[14px] border border-[rgba(229,224,219,0.4)] bg-white/60 p-[0.85rem]">
                      {item.productImage ? (
                        <img src={item.productImage} alt="" className="h-[60px] w-[60px] shrink-0 rounded-xl object-cover" />
                      ) : (
                      ...
                      <div className="flex min-w-0 flex-1 flex-col justify-center">
                        ...
                        <p className="mt-0.5 text-[0.81rem] text-[#9B9590]">单价: ¥{item.unitPrice.toFixed(2)}</p>
                ...
                <div className="flex flex-col gap-2">
                  {orderData.items?.map((item) => (
                    <div key={item.itemId} className="flex gap-[0.85rem] rounded-[14px] border border-[rgba(229,224,219,0.4)] bg-white/60 p-[0.85rem]">
                      {item.productImage ? (
                        <img src={item.productImage} alt="" className="h-[60px] w-[60px] shrink-0 rounded-xl object-cover" />
                      ) : (
                        <div className="flex h-[60px] w-[60px] shrink-0 items-center justify-center rounded-xl bg-[linear-gradient(135deg,#F5F2EE,#EDE8E3)] text-[#B5AEA8]">
                      ...
                      <div className="flex min-w-0 flex-1 flex-col justify-center">
                        ...
                        <p className="mt-0.5 text-[0.81rem] text-[#9B9590]">单价: ¥{item.unitPrice.toFixed(2)} × {item.quantity}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {(orderData.paymentNo || orderData.refundedAmount) && (
                <div className="grid grid-cols-2 gap-3">
                  {orderData.paymentNo && <InfoCell label="支付单号" value={<span className="font-mono text-[0.8rem]">{orderData.paymentNo}</span>} />}
              ...
              {orderData.remark && (
                <div className="rounded-[14px] border border-[rgba(229,224,219,0.4)] bg-white/60 px-4 py-[0.85rem]">
                  ...
                  <p className="whitespace-pre-wrap text-[0.87rem] leading-relaxed text-[#4A4540]">{orderData.remark}</p>
                </div>
              )}
              {orderData.cancelReason && (
                <div className="rounded-[14px] border border-[rgba(244,63,94,0.15)] bg-[linear-gradient(135deg,rgba(244,63,94,0.04),rgba(251,113,133,0.02))] px-4 py-[0.85rem]">
                  ...
                  <p className="whitespace-pre-wrap text-[0.87rem] leading-relaxed text-[#4A4540]">{orderData.cancelReason}</p>
                </div>
              )}
            </div>
          ) : (
