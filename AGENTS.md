...
> **定位**：LLM × DDD 工程化实战项目 — 在 DDD 六边形架构里集成 LLM，让 AI 链路可换供应商、可降级、可观测。**业务**：C2C 资产流转（固定价格 + 直发 + 平台不碰货），把复杂度留给架构与 AI 工程化。**工程亮点**：DDD 六边形 + CQRS · Saga · 事件驱动 · AI Port/Adapter 多级缓存 + 令牌桶限流 + stale 降级 · ES 搜索 + IK 分词 · ArchUnit 架构守卫 · 1,219 测试（JaCoCo 覆盖率 + PIT 变异测试双重质量门禁）。**2025 年 11 月启动**。
...
- **测试统计**：后端 11 模块合计 1,219 测试用例 / 1,152 注解 / 137 文件（2026-07-26 WSL2 实测全绿，`mock-maker-subclass` 模式）；前端 100 测试文件/950 测试用例。数字单一来源见 [doc/工程指标.md](doc/工程指标.md) §1.2
...
- **全量 Long→String 迁移**: 涉及所有模块——领域事件、值对象、DO、DTO、Port 接口、Adapter、Controller、测试文件。`SecurityContextUtil.getCurrentUserIdOrThrow()` 返回 `String`（原 Long）。`SnowflakeConfig`/`IdGenProperties` 等 Snowflake 配置已彻底移除，仅保留 `UuidV7IdGenerator`。`TestSecurityUtil.setSecurityContext()` 同时保留 `Long` 和 `String` 重载。全项目 2,169 测试通过（1,219 后端 + 950 前端）。
