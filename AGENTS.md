...
> **定位**：LLM × DDD 工程化实战项目 — 在 DDD 六边形架构里集成 LLM，让 AI 链路可换供应商、可降级、可观测。**业务**：C2C 资产流转（固定价格 + 直发 + 平台不碰货），把复杂度留给架构与 AI 工程化。**工程亮点**：DDD 六边形 + CQRS · Saga 状态机 · 事件驱动 + Outbox + DLQ 三级重试 · AI 7 件套（Port/Adapter + 多级缓存 + Redisson 令牌桶 + stale 降级 + AiMetrics + Prompt YAML + TokenBudget + Bulkhead）· ES 搜索 + IK 分词 · ArchUnit 架构守卫 · 2,427 测试（后端 1,389 + 前端 1,038）/ Domain 层 84.1% 行覆盖（JaCoCo + PIT 变异测试双重质量门禁）· SpringDoc OpenAPI 3 · Biome 0 errors。**2025 年 11 月启动**。
